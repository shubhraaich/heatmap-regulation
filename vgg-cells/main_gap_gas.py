...
#import torchvision.transforms as transforms
import transforms_cells
import datasets_cells
...
parser = argparse.ArgumentParser(description='PyTorch cells Training')
...
def main():
    ...
    if args.arch.startswith("vgg") :
        model = vgg_gap_gas(pretrained=True, input_size=(32,32));
...
#    std=[0.229, 0.224, 0.225];
    ...
    if args.train == 1 :
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets_cells.ImageFolder(img_dir=img_dir, gam_dir=gam_dir, ann_dir=ann_dir,
                transform_joint=transforms_cells.Compose_Joint([
                    transforms_cells.RandomHorizontalFlip(),
                    transforms_cells.RandomVerticalFlip(),
                ]),
                transform=transforms_cells.Compose([
                    transforms_cells.ToTensor(),
                    transforms_cells.Normalize(mean=mean, std=std),
                ]),
                gam_transform=transforms_cells.Compose([
                    transforms_cells.ToTensor2D(),
                ]),
            ),
            batch_size=args.batch_size, shuffle=True,
            num_workers=args.workers, pin_memory=True);

        if img_dir.endswith("N32") or img_dir.endswith("N50") :
        ...
        for epoch in range(args.start_epoch, args.end_epoch+1):
            ...
            # run validation code
            if img_dir.endswith("N32") or img_dir.endswith("N50") :
                end_exec = validate(img_dir, ann_dir, model, val_transform, epoch);
#                if end_exec : # validation error goes up
#                    break;
...
def validate(img_dir, ann_dir, model, val_transform, epoch) :
    postfix_ = 32 if img_dir.endswith("32") else 50;
    img_dir = os.path.join(os.path.dirname(img_dir), "val_N" + str(postfix_));
    ann_dir = os.path.join(os.path.dirname(ann_dir), "count_val_N" + str(postfix_) + ".mat");
    val_dif_fname = 'val_diff_old';

    images = datasets_cells.make_dataset(img_dir=img_dir,
                                         gam_dir=None,
    ...
    for i, (img_path, gam_path, target) in enumerate(images):
        # measure data loading time
        input = val_transform(datasets_cells.default_loader(img_path)).unsqueeze(0);
    ...
    if epoch == 1:
        ...
        if float(diff_old[-1]) - diff_abs < 0 :
            end_exec = True;
...
def test(test_transform, model, arch_type, load_epoch, img_dir, gam_dir, ann_dir) :
    ...
    images = datasets_cells.make_dataset_test(img_dir, gam_dir, ann_dir);
    time_start = time.time()
    ...
    for i, (img_path, gam_path, target) in enumerate(images):
        ...
        input = test_transform(datasets_cells.default_loader(img_path)).unsqueeze(0);
        input_var = torch.autograd.Variable(input, volatile=True).cuda();

        # compute output
        output_count, output_cam = model(input_var);
        output_cam = output_cam.data.cpu().view(output_cam.size(0), 32, 32).squeeze();
        pred = int(round(output_count.data.cpu()[0,0]));

...
def make_dataset(img_dir, gam_dir, ann_dir, is_copy=True):
    count_all = np.squeeze(scipy.io.loadmat(ann_dir)['count_gt'].astype('i2'));
    num_copies = 100; # keep small for validation
    images = []
    img_dir = os.path.expanduser(img_dir);
    for i in xrange(len(os.listdir(img_dir))):
